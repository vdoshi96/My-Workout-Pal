...
function setMeasurement(
  ...
  if (set.kind === "weight_reps") {
    return `${set.weightKg === undefined ? "Weight not recorded" : formatInsightWeight(set.weightKg, unitSystem)} · ${repetitions}`;
  }
  if (set.kind === "bodyweight_reps") return set.repetitions === undefined ? repetitions : `${set.repetitions} bodyweight reps`;
  ...
  return (
    <article className="insights-page history-detail" aria-labelledby="history-detail-title">
        <header className="insights-heading contour-surface">
          <div>
            <Link className="back-link" href="/app/history"><Icon name="arrow-left" /> Back to history</Link>
            ...
            <p>{formatHistoryDate(session.occurredAt, timezone)} · {formatInsightDuration(session.durationSeconds)}</p>
          </div>
        </header>

        <aside className="archive-notice">
          <strong>Read-only snapshot.</strong> Exercise names, substitutions, notes, and logged values are preserved from this session.
        </aside>
            ...
            <li key={exercise.id}>
              <header>
                ...
                <div>
                  ...
                  <small>{exercise.sectionKind} · {exercise.status}</small>
                </div>
              </header>
              {exercise.substitutionReason ? <p className="history-substitution">Substitution: {exercise.substitutionReason}</p> : null}
              {exercise.note ? <p className="history-note"><strong>Workout note:</strong> {exercise.note}</p> : null}
              {exercise.sets.length === 0 ? (
                <p className="history-no-sets">No sets were logged for this {exercise.status} movement.</p>
              ) : (
                <ol className="history-sets">
                  {exercise.sets.map((set) => (
                    <li key={set.id}>
                      ...
                      <small>{set.setKind === "warmup" ? "Warm-up" : "Work set"}{set.formRating ? ` · Form ${set.formRating}/5` : ""}</small>
        ...
        {session.cardio ? (
          <section className="history-cardio" aria-labelledby="history-cardio-title">
            ...
            <h2 id="history-cardio-title">{session.cardio.mode === "runner" ? "Runner" : "Walker"} cardio</h2>
            <dl>
              <div><dt>Time</dt><dd>{formatInsightDuration(session.cardio.durationSeconds)}</dd></div>
              <div><dt>Distance</dt><dd>{session.cardio.distanceMeters === undefined ? "Not recorded" : formatInsightDistance(session.cardio.distanceMeters, unitSystem)}</dd></div>
              <div><dt>Incline</dt><dd>{session.cardio.inclinePercent === undefined ? "Not recorded" : `${session.cardio.inclinePercent}%`}</dd></div>
            </dl>
          </section>
        ) : null}
    </article>
