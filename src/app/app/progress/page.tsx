    ...
    <section className="insights-page" aria-labelledby="progress-title">
      <header className="insights-heading contour-surface">
        <div>
          ...
          <p>Completed workouts only, grouped in {progress.preferences.timezone}. Interrupted sessions stay visible in History but never inflate these totals.</p>
        </div>
        <Link className="insight-action" href="/app/prs">Personal records <Icon name="arrow-right" /></Link>
      </header>

      <dl className="progress-totals">
        <div><dt>Completed workouts</dt><dd>{progress.totals.completedSessions}</dd></div>
        <div><dt>Training volume</dt><dd>{formatInsightVolume(progress.totals.volumeKg, unitSystem)}</dd></div>
        <div><dt>Logged duration</dt><dd>{formatInsightDuration(progress.totals.durationSeconds)}</dd></div>
        <div><dt>Cardio distance</dt><dd>{formatInsightDistance(progress.totals.distanceMeters, unitSystem)}</dd></div>
      </dl>

      <aside className="progress-integrity">
        <strong>{progress.projection.state === "persisted" ? "Saved rollups detected." : "Calculated from saved logs."}</strong>{" "}
        The chart below is rebuilt from your completed set and cardio logs. {progress.totals.abandonedSessions} interrupted workout{progress.totals.abandonedSessions === 1 ? " is" : "s are"} excluded.
      </aside>
      ...
      {progress.series.length === 0 ? (
        <div className="member-empty-sheet">
          ...
          <p>No sample points are mixed into account analytics. Any interrupted session remains recoverable or archived separately.</p>
          <Link href="/app">Open your program</Link>
        </div>
      ) : (
        <section className="progress-timeline" aria-labelledby="progress-timeline-title">
          <header className="section-heading">
            <div><span className="eyebrow">Daily series</span><h2 id="progress-timeline-title">Training timeline</h2></div>
              ...
              return (
                <li key={point.date}>
                  ...
                  <div className="progress-sources">
                    {point.sourceIds.map((sessionId) => (
                      <Link href={`/app/history/${sessionId}`} key={sessionId} aria-label={`Open saved workout from ${formatProgressDate(point.date)}`}>
                        <Icon name="history" />
                      </Link>
                    ))}
                  </div>
