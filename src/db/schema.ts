...
export const programDays = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("program_days_key_unique").on(table.ownerFirebaseUid, table.revisionId, table.dayKey),
    check("program_days_number_shape", sql`${table.dayNumber} between 1 and 14`),
  ],
);
...
export const programSections = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("program_sections_key_unique").on(
      table.ownerFirebaseUid,
      table.revisionId,
      table.sectionKey,
    ),
...
export const programPrescriptions = pgTable(
  "program_prescriptions",
  {
    ...
    revisionId: uuid("revision_id").notNull(),
    sectionId: uuid("section_id").notNull(),
    prescriptionKey: varchar("prescription_key", { length: 40 }).notNull(),
  ...
  (table) => [
    ...
    uniqueIndex("program_prescriptions_key_unique").on(
      table.ownerFirebaseUid,
      table.revisionId,
      table.prescriptionKey,
    ),
...
export const programCardioPrescriptions = pgTable(
  "program_cardio_prescriptions",
  {
    ...
    revisionId: uuid("revision_id").notNull(),
    dayId: uuid("day_id").notNull(),
    cardioKey: varchar("cardio_key", { length: 40 }).notNull(),
    displayOrder: integer("display_order").notNull(),
    mode: cardioMode("mode").notNull(),
    durationSeconds: integer("duration_seconds").notNull(),
    distanceM: numeric("distance_m", { precision: 12, scale: 3, mode: "number" }),
  ...
  (table) => [
    ...
    uniqueIndex("program_cardio_prescriptions_key_unique").on(
      table.ownerFirebaseUid,
      table.revisionId,
      table.cardioKey,
    ),
    uniqueIndex("program_cardio_prescriptions_order_unique").on(
      table.ownerFirebaseUid,
      table.revisionId,
      table.dayId,
      table.displayOrder,
    ),
    check("program_cardio_display_order_shape", sql`${table.displayOrder} between 1 and 2`),
