...
- Settings sign-out clears only the current Firebase UID’s IndexedDB runner namespace before deleting the same-origin secure session, and signs out the Firebase client when public configuration exists. Account deletion remains truthfully disabled until the deletion saga and configured Firebase reauthentication paths are implemented and verified.
- The program-editor persistence contract now validates a strict owner-free five-day graph and publishes edits as one owner-scoped immutable revision transaction. It derives logging meaning from canonical or owner custom exercises, enforces the active equipment profile, preserves source metadata only for the same exercise identity, rejects stale bases and foreign references, and replays identical idempotency keys without another revision. A private same-origin endpoint is connected with authentication before body or storage access; the editor UI is not connected yet.
