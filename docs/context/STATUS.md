...
- The isolated workout branch passes 31 test files and 263 tests serially, strict TypeScript, full ESLint, Drizzle metadata validation, generated-service-worker parity, documentation parity, and the Next.js Webpack production build. Resume hydration and reconciliation contribute 16 tests, including direct compatibility with the real PGlite repository read model. A local production-server smoke observed `401` with `no-store` for an unauthenticated resume and `403` with `no-store` for a cross-origin start.
...
- `/private/tmp/mwp-workout-repository` on `agent/workout-repository`: the owner-scoped repository, private API, browser adapter, and resume recovery are verified through local commit `7f941a7` and await explicit merge authorization. This is the only non-main worktree.
