...
- The program-editor persistence contract now validates a strict owner-free five-day graph and publishes edits as one owner-scoped immutable revision transaction. It derives logging meaning from canonical or owner custom exercises, enforces the active equipment profile, preserves source metadata only for the same exercise identity, rejects stale bases and foreign references, and replays identical idempotency keys without another revision. The route and editor UI are not connected yet.
