...
- `pnpm typecheck`, `pnpm lint`, 88 unit and integration tests, generated-service-worker parity, and `pnpm docs:check` pass before the pending worktree integrations.
...
- Neon database credentials are absent and will be created through the Vercel Marketplace workflow.
- `YOUTUBE_API_KEY` is absent. Video discovery, mechanical eligibility checks, and final approval cannot complete without it.
