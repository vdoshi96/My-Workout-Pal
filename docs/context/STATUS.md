...
- The program editor now maps the active owner-scoped revision into a strict unpublished draft, supports program/day names, prescription targets, notes, rest, sets, keyboard-equivalent reorder buttons, and walker/runner defaults, and publishes through the private endpoint. It warns before leaving dirty work, preserves the idempotency key across retries, exposes the separate equipment confirmation control, and labels unsaved, validation, conflict, saving, and published states honestly. Adding or replacing exercises remains the next editor increment.
