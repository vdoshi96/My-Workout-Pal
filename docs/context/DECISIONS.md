...
## 2026-08-27: Release Wave 0 only with migration 0005

The user authorized publication of the selected Corner Companions packet and the exact Wave 0 integration commit. GitHub `main` and the public integration branch retain application commit `e44c1328bb566e1e2af5072f178648cefad736b5`.

Production migration `0005_flexible_routine_topology` must precede application deployment. Its non-null topology keys make the older application write-incompatible even though its read paths can tolerate the expanded schema. A diagnostic rollback proved that the older onboarding path returns `500` because it doesn't supply `section_key`, `prescription_key`, or `cardio_key`. Therefore, after migration `0005`, rollback means restoring the verified recovery point and the older application together, or keeping schema `0005` and serving the exact Wave 0 application.

The original hosted failure label was inaccurate. Session creation and secure-cookie verification passed; onboarding also returned `201`. The verifier retained a stale fixed-five-day summary assertion and did not distinguish onboarding from session setup. The corrected verifier uses the flexible summary contract, reports separate stages, and covers custom publication, reload, zero-or-one cardio, stable day keys, start-or-resume conflict reuse, equipment revision, immutable history, foreign-owner equivalence, and cleanup. Keep production on exact application commit `e44c1328bb566e1e2af5072f178648cefad736b5`; later verifier and documentation commits don't change the deployed application tree.
