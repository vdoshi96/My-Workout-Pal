...
**Navigation and UI states:** This slice adds a private operator command that reads an ignored browser-candidate artifact and imports it into the ignored curation checkpoint. It adds no application page, route, or public control. Command output distinguishes imported, already present, invalid, wrong target, partial browser coverage, pending hydration, hydrated, unavailable, and provider-blocked candidates. Each proposed pair reports `api-discovery-complete`, `browser-discovery-complete`, or `discovery-incomplete`. A target may become ready for review through the authorized browser fallback only after the complete browser manifest and official hydration gates pass; the report never describes that fallback as API relevance- or view-count-ordered discovery.

**Domain types and invariants:** Each browser artifact contains a query-run manifest plus candidate observations. A query run names the canonical exercise slug, variation ID, exact deterministic query, observation timestamp, count of fully traversed standard-video cards, completion flag, and provenance `browser-rendered-search`. Each candidate observation adds its normalized standard watch URL, rendered title, channel, duration text, optional visible-view text, exact result position, and the matching query-run timestamp. The importer accepts only catalog-derived target keys, every query produced by the target's three movement/alias variants, complete standard-card position ranges, and normalizable non-Shorts YouTube watch URLs. It derives the durable video ID at the boundary and deduplicates the same scoped candidate while retaining distinct query observations. Browser observations use a separate provenance collection; they do not create official relevance or `viewCount` query keys, increment API search accounting, imply `syndicated: true`, or replace hydrated metadata. Visible view text is non-authoritative discovery evidence and never becomes durable seed truth. Partial query coverage remains `discovery-incomplete`. Full browser coverage becomes `browser-discovery-complete` only when all three required query runs are complete and every imported ID from those runs has a completed official `videos.list` check. After the existing relevance, quality, safety, equipment, availability, embeddability, region, duration, and redundancy gates, hydrated current view count orders eligible candidates descending; relevance score remains a gate diagnostic rather than an ordering key.

**Persistence, authentication, and authorization:** The input, checkpoint, report, and import receipt remain mode-`0600` files under ignored `.local/youtube-curation/` and are replaced atomically. The receipt durably stores browser query runs and candidate observations separately from official API query state. The command has no Firebase user identity and no user-owned data boundary; authorization is local operator access to the repository and ignored state. It does not write Neon, Firebase, Vercel, production seeds, public documentation, completed official query keys, or API search counters. It reads the existing local YouTube API key only when official hydration is requested.
