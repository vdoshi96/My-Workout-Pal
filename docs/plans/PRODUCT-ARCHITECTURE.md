...
**Navigation and UI states:** This slice adds a private operator command that reads an ignored browser-candidate artifact and imports it into the ignored curation checkpoint. It adds no application page, route, or public control. Command output distinguishes imported, already present, invalid, wrong target, pending hydration, hydrated, unavailable, and provider-blocked candidates. Browser imports augment the report but never change a target from `discovery-incomplete` to `ready-for-review`; only completed official query keys can do that.

**Domain types and invariants:** Each browser record contains schema version, canonical exercise slug, variation ID, normalized standard watch URL, rendered title, channel, duration text, optional visible-view text, the exact search text, result position, discovery timestamp, and provenance `browser-rendered-search`. The importer accepts only catalog-derived target keys and normalizable non-Shorts YouTube watch URLs, derives the durable video ID at the boundary, and deduplicates the same scoped candidate while retaining distinct query observations. Browser observations use a separate provenance collection; they do not create official relevance or `viewCount` query keys, increment API search accounting, imply `syndicated: true`, or replace hydrated metadata. Visible view text is non-authoritative discovery evidence and never becomes durable seed truth. Imported candidates remain pending hydration, then reuse the existing official `videos.list` hydration, availability, embeddability, region, duration, mechanical eligibility, ranking, and manual-review gates.

**Persistence, authentication, and authorization:** The input, checkpoint, report, and import receipt remain mode-`0600` files under ignored `.local/youtube-curation/` and are replaced atomically. The command has no Firebase user identity and no user-owned data boundary; authorization is local operator access to the repository and ignored state. It does not write Neon, Firebase, Vercel, production seeds, public documentation, completed official query keys, or API search counters. It reads the existing local YouTube API key only when official hydration is requested.

**Loading, empty, error, interrupted, and worst-case states:** A missing or empty artifact produces a truthful no-op. Schema mismatches, malformed timestamps, invalid URLs, Shorts, unknown targets, negative positions, and conflicting scoped observations fail validation before checkpoint mutation. Reimport is idempotent. An interrupted atomic write preserves the prior checkpoint. If official hydration is unavailable, quota-blocked, or omits an ID, the browser observation remains resumable and pending or records the existing checked-unavailable state without inventing metadata. Imported candidates cannot erase prior official query provenance, hydration, rejection, review, or approval state. A browser result that later proves private, removed, non-embeddable, wrong movement/equipment, unsafe, redundant, or otherwise ineligible follows the existing stable rejection path.
...
**Acceptance criteria, automated tests, and browser evidence:** Retain a failing test showing that a valid browser record cannot yet be imported, then pass it with normalized scoped insertion, preserved observation provenance, and no official query completion or search-unit increase. Retain failures for duplicate import, Shorts, unsupported targets, malformed timestamps, conflicting scoped records, and interrupted persistence. Prove an imported ID is hydrated through `videos.list`, retains `syndicationEvidence: "unknown"` unless independently verified, and remains `discovery-incomplete` until all official query keys complete. Verify strict TypeScript, scoped lint, the complete YouTube test matrix, documentation parity, and a private command smoke using the rendered standard-video pool. Metadata-only browser evidence never satisfies approval; each selected video must still be played visually and audibly from start to finish before manual approval, exact-two validation, embed/direct-URL checks, or production seeding.
