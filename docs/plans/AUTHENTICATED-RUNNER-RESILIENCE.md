...
- A server-confirmed completed or skipped exercise decision also outranks stale unsent set, note, and decision mutations for that exact exercise. Reconciliation preserves already confirmed set and note history but prevents a suspended tab from reopening the exercise or blocking terminal completion.
...
- Raw field tokens that have not been activated with **Save activity** remain unsent local editing projections. They may persist for same-device refresh recovery, but they are not operations and are never submitted. Atomic record reconciliation may replace them with a server-confirmed projection. Navigation protection continues to guard raw dirty fields.
...
- A confirmed exercise completion or skip supersedes stale exercise-local mutations in another tab without deleting already saved history.
...
Product checkpoint `83f65f2519e5c2a77139fe9df855c8c0260536fb` established the initial non-browser gate. Test-evidence checkpoint `089df2f0790973a63e1800d878d551db6a57ff35` adds only retained runner screenshots and guest discovery time budgeting. Primary-review source checkpoint `27d85a93031e5e4570b5e4b2743a1fa342bf2893` closes the stale raw-projection edge with a retained three-test failure and a 30-test passing storage matrix. Exact browser checkpoint `f7cbe85d377c777e51520feec4e9348b508f5bfe` corrects the raw-note expectation and passes the complete authenticated matrix with 29 cases passed and one intentional engine-scoped skip. Test-policy checkpoint `f0d888f09af143ecce2a0f7ef82f11f2e64adb5f` binds the plan assertion to the same verified refresh-recovery behavior and passes the current aggregate gate with 90 files and 633 tests plus all database, seed, PWA, documentation, build, and production-boundary checks. Its public matrix passes 43 cases with one documented WebKit service-worker skip. The newest QA record retains the four distinct synthetic states and documents the remaining hosted/provider gates.

Large local gates use one build artifact set at a time. After reviewed evidence is retained, remove the root `.next`, fixture `.next-authenticated`, `test-results`, and `playwright-report` directories. Keep the repository's ignored dependency store during active development so pnpm doesn't recreate a temporary store for every worktree.
