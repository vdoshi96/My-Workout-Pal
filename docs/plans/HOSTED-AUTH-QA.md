...
## Completed email-action evidence

Exact source checkpoint `fd9c051` ran the opt-in Chromium `1440×1000` lifecycle against synchronized production runtime `f6b0ad6fa5397e51bcd0028fc11de9f619f09e21`, Ready as `dpl_3iYkkQSSzvYeYyFEvHX3WbAyJHhT`. It created one application identity and one purpose-separated action-code identity on `example.invalid`. Firebase accepted the application's verification and recovery requests. The trusted runner then generated the second identity's verification/reset links without dispatching email, parsed and applied both codes through the official REST boundary, confirmed exact UID verification, rejected the old password, accepted the recovered password through the public sign-in surface, verified the secure cookie, and proved revocation denial.
