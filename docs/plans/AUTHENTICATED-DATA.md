...
## Live Firebase configuration checkpoint

Use Firebase project `my-workout-pal-92819` as the only identity project for this release. The earlier proposed project ID was never created and has no application or credential state to migrate. Keep the project on Spark. Do not enable Identity Platform, phone authentication, Firebase Hosting, Firestore, Storage, Analytics, or another billable service for this release.

Register one web app named `My Workout Pal Web`. Configure Email/Password and Google only. Keep email-link sign-in off. Set a reviewed public project name and support email for Google, and authorize `localhost`, the Firebase default domain, and the deployed `my-workout-pal-chi.vercel.app` hostname. Add a preview hostname only when Firebase accepts an exact stable host; do not assume wildcard-domain support.

Store the four public values in `NEXT_PUBLIC_FIREBASE_API_KEY`, `NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN`, `NEXT_PUBLIC_FIREBASE_PROJECT_ID`, and `NEXT_PUBLIC_FIREBASE_APP_ID`. Store the Admin values in `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, and `FIREBASE_PRIVATE_KEY`. Keep every value in ignored local and encrypted Vercel environment storage. Confirm names and presence without printing values. The public API key is intentionally browser-visible, but it remains configuration data and must not appear in committed QA reports.

Treat partial setup as unavailable. If one public value is missing, the sign-in page keeps the truthful configuration gate. If an Admin value is missing or malformed, session exchange and permanent server mutations fail closed before database access. If the client and Admin project IDs differ, stop release verification and correct the environment instead of accepting tokens across projects.

Acceptance requires both providers enabled, exact authorized domains, successful local and deployed public/Admin configuration detection, and no secret value in Git history, build logs, runtime logs, documentation, or browser evidence. Browser evidence must use disposable password and Google identities to cover registration, duplicate and invalid input, email verification, recovery, sign-in, sign-out, expired and revoked sessions, CSRF, cross-user denial, recent reauthentication, deletion, and the completion reconciler's dry-run and approved one-owner apply paths. Remove disposable identities and owned database rows after the evidence is complete.
