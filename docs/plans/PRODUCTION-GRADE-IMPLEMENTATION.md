...
The full matrix exposed stale navigation in that same journey: navigation inherited an older timestamp, so a stale tab's later save could replace the newly selected set. Timestamp actual set or exercise navigation at the UI boundary, preserving explicit operation times and the existing merge rules. Do not timestamp every draft edit: the existing divergent-tab journey showed that doing so lets another tab's newer unsaved draft replace the first tab's value before both saves. Verify both tab journeys without changing their assertions. The main-passing WebKit Quiet Set journey also reported hydration errors in Settings because Node and WebKit expose different IANA lists. Generate the Settings choices on the server and pass that list through the existing page and fixture to the form. Keep the helper's full-list and saved-alias contract; do not suppress hydration errors or alter their assertions.
