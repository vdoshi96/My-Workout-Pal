...
The user's existing Firefox session then opened the exact protected preview directly, without changing deployment protection. Each corrected representative route rendered two approved tabs, visible title/channel attribution, a direct fallback, and one `youtube-nocookie` iframe at a time. Switching to Demo 2 produced the interaction-rendered second fallback, live English captions, and Firefox's playing indicator for both `mHBOUz9KY9A` and `MprE4ppd27U`, proving real playback of the corrected selections. The later production guest replay independently proved the same two-tab/attribution/direct-fallback/one-iframe contract and Demo 2 switching on the public alias, but did not capture a production playing indicator. Together these are catalog-complete SSR, representative protected-preview playback, and production UI integration; they do not claim the remaining unavailable-fallback, authenticated-runner, or production playback-indicator matrix.

## Remaining release evidence

- Extend representative playback to the authenticated runner and unavailable-first fallback while preserving keyboard, reduced-motion, dark-mode, reflow, and Axe behavior.
- Capture a real playing indicator and unavailable-first fallback on the current public production alias, then inspect runtime logs.
