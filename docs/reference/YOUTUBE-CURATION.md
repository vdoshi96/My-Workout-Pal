...
Ignored `.env.local` contains `YOUTUBE_API_KEY`; bounded official API discovery and hydration ran without printing the key on August 26, 2026. The key is not copied to Vercel because it is a local curation credential, not an application runtime dependency. Official discovery stopped at 102 successful search requests with 60 of 162 searches remaining after reset. The first v1 import attempt rejected a gapped run; after private correction, its 29 runs and 195 observations imported and were hydrated before review exposed the lazy 5–8-card completeness overclaim. The hardened schema-v2 replacement has now atomically superseded that receipt with 29 exact deduplicated query runs, 435 standard-watch observations, 15 contiguous cards per run, and all 10 expected fallback targets. It preserved existing checkpoint candidates and used only four further `videos.list` calls. The private checkpoint now records 37 hydration calls, 139 total estimated units, 1,792 target-scoped candidates, 1,674 hydrated unique IDs, zero unavailable IDs, and zero pending checks. The regenerated report truthfully returns `ready-for-review`: 17 targets are API-discovery-complete, 10 are browser-window-complete, eight pairs are ready for review, 10 need a second candidate, and nine are privately `approved-for-seed`. Fully watched and scoped-approved pairs are dumbbell bench (`ZzFblmTUxYU`, `YwrzZaNqJWU`), seated dumbbell shoulder press (`b5JzUH8gsOg`, `poD_-zaG9hk`), incline dumbbell press (`uJA9dx_MMyA`, `gl8H4QLXKTo`), overhead dumbbell triceps extension (`m4ujXuSuQ0I`, `0h0hHCZEqQw`), dead bug (`rbemelnkHag`, `kwWZBbkXtg4`), front plank (`wCBOqf-HrTI`, `GQE8ASRA7t0`), barbell bent-over row (`vT2GjY_Umpw`, `-xlBxIMqh3A`), reverse lunge (`qYGzE8Uou7E`, `AXxjaK_KtJY`), and standing calf raise (`gwLzBJYoWlI`, `wxwY7GXxL4k`). Every approved candidate has scoped embed evidence; short-duration candidates receive active Shorts-path checks. Verified Shorts `MKZIuwc-VCw`, `2D0TyoHv_EY`, and `7imlMBQIcjk` are durably rejected and absent from ranking. Nothing is checked into the production seed, and no other mapping is complete. Remaining full-watch decisions, catalog-wide exact-two approval, production seed, and live production embed verification remain release gates.
