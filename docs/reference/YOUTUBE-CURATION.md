...
Ignored `.env.local` contains `YOUTUBE_API_KEY`; bounded official API discovery and hydration ran without printing the key on August 26, 2026. The key is not copied to Vercel because it is a local curation credential, not an application runtime dependency. Official discovery stopped at 102 successful search requests with 60 of 162 searches remaining after reset. The first v1 import attempt rejected a gapped run; after private correction, its 29 runs and 195 observations imported and were hydrated before review exposed the lazy 5–8-card completeness overclaim. The hardened schema-v2 replacement atomically superseded that receipt with 29 exact deduplicated query runs, 435 standard-watch observations, 15 contiguous cards per run, and all 10 expected fallback targets. It preserved existing checkpoint candidates and used only four further `videos.list` calls. The private checkpoint records 37 hydration calls, 139 total estimated units, 1,792 target-scoped candidates, 1,674 hydrated unique IDs, zero unavailable IDs, and zero pending checks. The final zero-request report has 17 API-discovery-complete targets, 10 browser-window-complete targets, 27 `approved-for-seed` pairs, 54 selected unique IDs, and no remaining mapping. Every selected candidate has full-watch review and scoped localhost privacy-enhanced embed, direct-fallback, visible-control, and keyboard evidence; short-duration candidates received active Shorts-path checks, and verified Shorts remain durably rejected outside ranking.

The deterministic schema-one production manifest is checked in with 54 globally unique rows and exactly the ten permitted durable fields; regeneration is byte-stable and catalog-wide `seed:check` passes. It is applied to Neon, where idempotent replay and read-only verification pass. Catalog-complete protected-preview SSR and representative dual-playback evidence exist; the complete hosted preview matrix and public-production dual-embed replay remain release gates.
