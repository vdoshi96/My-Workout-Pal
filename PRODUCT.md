...
The main member navigation is Today, Routine, Library, and Progress. Settings remains a utility and works before routine setup. It contains the character preference, units, time zone, motion preference, and account controls. Library is directly accessible before and after routine setup. Before setup it shows the explicitly labeled dumbbell, bodyweight, and bench catalog. Today prioritizes Start or Resume. The runner shows the current movement, targets, and entry controls before expandable guidance. Log set & rest queues one intended set and starts its configured deadline; repeating the unchanged action preserves its identity and deadline. Next set and Next exercise are explicit and keep the rest timer running. Entries prefill from the same movement and set phase. Pending, confirmed, and failed saves stay distinguishable; confirmed device writes allow leaving with changes queued for sync. Ending a workout or skipping a movement requires confirmation.
