...
import { GET as productionCsrf } from "@/app/api/auth/csrf/route";
import {
...
import { GET as fixtureCsrf } from "../../tests/fixtures/authenticated-app/app/api/auth/csrf/route";
...
describe("credential-free authenticated harness boundary", () => {
  it("derives only bounded synthetic server viewers and scenarios from fixture headers", () => {
    ...
    expect(
      harnessRequestContext(
        new Headers({ [HARNESS_VIEWER_HEADER]: "alice-unverified" }),
      ),
    ).toMatchObject({
      scenario: "ready",
      viewer: {
    ...
    expect(
      harnessRequestContext(new Headers({ [HARNESS_VIEWER_HEADER]: "bob" }))
        .viewer?.uid,
    ...
    expect(
      harnessRequestContext(
        new Headers({ [HARNESS_VIEWER_HEADER]: "attacker-chosen-uid" }),
      ),
    ).toEqual({ scenario: "ready", scope: "default", viewer: null });
    expect(
      harnessRequestContext(
        new Headers({ [HARNESS_SCENARIO_HEADER]: "unknown-mode" }),
      ),
  ...
  it("mounts the real day, workout, workout API, and history vertical slice only in the fixture", () => {
    const fixtureRoot = resolve(
      repositoryRoot,
      "tests/fixtures/authenticated-app/app",
    );
  ...
  it("adapts only the harness CSRF cookie before delegating the untouched workout body", async () => {
    ...
    expect(adapted.cookies.get(HARNESS_CSRF_COOKIE_NAME)?.value).toBe(
      "fixture-token",
    );
  ...
  it.each([
    ["production", productionCsrf, CSRF_COOKIE_NAME],
    ["fixture", fixtureCsrf, HARNESS_CSRF_COOKIE_NAME],
  ] as const)(
    "%s CSRF bootstrap reuses a valid cookie across concurrent tabs",
    async (_label, getter, cookieName) => {
      const existing = "A".repeat(43);
      const response = (
        getter as unknown as (request: NextRequest) => Response
      )(
        new NextRequest("https://example.test/api/auth/csrf", {
          headers: { cookie: `${cookieName}=${existing}` },
        }),
      );
  ...
  it("retains a confirmed terminal runner record until owner-scoped cleanup", () => {
    const ownerRunner = readFileSync(
      resolve(
        repositoryRoot,
        "src/components/workout/owned-workout-runner.tsx",
      ),
      "utf8",
    );
  ...
  it("returns no-store for a rejected private teardown request", async () => {
    ...
    try {
      const response = await deleteHarnessScope(
        new NextRequest("http://127.0.0.1:3110/api/harness/scope", {
          method: "DELETE",
        }),
      );
      ...
      if (previous === undefined)
        delete process.env["MWP_AUTHENTICATED_HARNESS"];
